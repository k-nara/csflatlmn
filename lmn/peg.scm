...
(define (test-parser parser str)
  ;; 文字列 STR をパーサー PARSER で解析し、その結果を出力する。
...
(define (($g parser :optional [name #f] [atomic #f]) stream last)
  ...
  ;; １．PARSER 全体に名前 NAME を与える。これはエラーメッセージを改善
  ;; する場合がある。２．PARSER が途中で失敗したとき、文字を消費しなかっ
  ;; たかのように振る舞う。これは "$or" のバックトラックの挙動に影響す
  ;; る。
  (receive (status value rest) (parser stream last)
    (cond [status
           (values #t value rest)]
