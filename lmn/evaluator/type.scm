...
;; stack-ref のドキュメントを参照) 。整数のリスト (M) の場合、検査対象
;; のプロセス文脈の第 K ポートは型検査が成功するように適当に選ばれ、
;; next を呼び出す前にそのポートが TC-LSTACK の M 番目にセットされる。
;; ARGS にリストが含まれているとき、型検査が成功するようなポートの選び
;; 方が複数ある場合がある。まだ候補が残っているときに next が boolean
;; を返した場合、別の候補を TC-LSTACK にセットし、ふたたび next を呼び
;; 出す。
