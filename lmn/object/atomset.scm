...
;; 未定義であるもの、あるいは引数 (＝ポートの partner) がその内部を指し
;; ているようなものは LMNtal プロセスとして不適切であるため、このような
;; <atomset> を ill-formed であると言って区別する。対して、 ill-formed
;; でない <atomset> は適切な LMNtal (部分) プロセスを表し、これらを部分
;; プロセスと呼ぶ。特に０価の部分プロセスをプロセスと呼ぶ。
